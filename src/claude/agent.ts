...
// Track current model per chat (default: bonsai)
...
/**
 * Strip proxy/routing banners injected by third-party API endpoints.
 * Removes the two known bonsai sentences that may appear anywhere in the text.
 */
function stripProxyBanners(text: string): string {
  return text
    .replace(/@bonsai:\s*routing to\s+[^.]*\./gi, '')
    .replace(/Enter\s+@switch\s+in your prompt to switch to another frontier model in stealth for free\./gi, '')
    .replace(/(?:🌱\s*)+/g, '')
    .replace(/\n{3,}/g, '\n\n')
    .trimEnd();
}
...
export async function sendToAgent(
  ...
  // Determine model to use (default to 'bonsai' to match getModel() default)
  const effectiveModel = model || chatModels.get(chatId) || 'bonsai';
    ...
    for await (const responseMessage of response) {
      ...
      if (responseMessage.type === 'assistant') {
        ...
        for (const block of responseMessage.message.content) {
          ...
          if (block.type === 'text') {
            fullText += block.text;
            onProgress?.(stripProxyBanners(fullText));
  ...
  return {
    text: stripProxyBanners(stripReasoningSummary(fullText)) || 'No response from Claude.',
    toolsUsed,
    usage: resultUsage,
...
export function getModel(chatId: number): string {
  return chatModels.get(chatId) || 'bonsai';
}
